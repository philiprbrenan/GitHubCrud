...
#D1 Using saved access tokens                                                   # Call methods directly using a saved access token rather than first creating a L<GitHub> description object and then calling methods using it. This is often more convenient if you just want to perform one or two actions using a L<perl> one liner. See: L<https://github.com/philiprbrenan/GitHubCrud/blob/main/.github/workflows/main.yml> for examples in action.
...
sub listRepositoryUsingSavedToken($$;$)                                         # List the files in a repository using a saved token
 {my ($userid, $repository, $accessFolderOrToken) = @_;                         # Userid on GitHub, repository name, optionally: location of access token.
  my $g = GitHub::Crud::new;
  $g->userid     = $userid;     $userid     or confess "Userid required";
  $g->repository = $repository; $repository or confess "Repository required";
...
Call methods directly using a saved access token rather than first creating a L<GitHub|https://github.com/philiprbrenan> description object and then calling methods using it. This is often more convenient if you just want to perform one or two actions using a L<Perl|http://www.perl.org/> one liner. See: L<https://github.com/philiprbrenan/GitHubCrud/blob/main/.github/workflows/main.yml> for examples in action.
...
=head2 listRepositoryUsingSavedToken   ($userid, $repository, $accessFolderOrToken)
...
19 L<listRepositoryUsingSavedToken|/listRepositoryUsingSavedToken> - List the files in a repository using a saved token

20 L<listWebHooks|/listWebHooks> - List web hooks associated with your L<GitHub|https://github.com/philiprbrenan> repository.

21 L<loadPersonalAccessToken|/loadPersonalAccessToken> - Load a personal access token by userid from folder L<personalAccessTokenFolder|/personalAccessTokenFolder>.
...
if (0) {                                                                        #TlistRepositoryUsingSavedToken
  listRepositoryUsingSavedToken("philiprbrenan", "test", "/home/phil/files/");
 }
